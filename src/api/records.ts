	...
	constructor (kCom: VCore<KI, KO>, vCom: VCore<VI, VO>, err?: string) {
		...
		this.addRule(makeRule((value) => {
			for (const k of value?.keys() ?? []) {
				...
				if (!kValid.valid) return isInvalid(err ? [err] : [`contains an invalid key ${String(k)}`], value)
				if (!vValid.valid) return isInvalid(err ? [err] : [`contains an invalid value for key ${String(k)}`], value)
				// @ts-ignore
