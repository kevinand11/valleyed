...
export const int = (err = 'is not an integer') =>
	standard<number, number>(
		({ input, context }) => `(${input} === parseInt(${input})) ? ${input} : ${context}.PipeError.root('${err}', ${input})`,
		{
			context: () => ({ PipeError }),
			schema: () => ({ type: 'integer' }),
		},
...
export const asRounded = (dp = 0) =>
	standard<number, number>(({ input, context }) => `Number(${input}.toFixed(${context}.dp))`, { context: () => ({ dp }) })
