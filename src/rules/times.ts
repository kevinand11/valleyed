...
export const isTime = <T extends Timeable>(error = 'is not a valid datetime') =>
	makeRule<T>((value) => {
...
export const isLaterThan = <T extends Timeable>(compare: Timeable, error = 'is not later than compared value') =>
	makeRule<T>((value) => {
		const val = value as T
...
export const isEarlierThan = <T extends Timeable>(compare: Timeable, error = 'is not earlier than compared value') =>
	makeRule<T>((value) => {
		const val = value as T
		const date = new Date(val)
		const compareDate = new Date(compare)
